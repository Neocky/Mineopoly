...
summon minecraft:creeper 7 64 -7 {Invisible:1b, NoGravity:1b, NoAI:1b, PersistenceRequired:1b, Tags:["menu_rent"]}
...
execute if score @s currentField matches 1 if entity @a[tag=property_brown_1] as @a[tag=property_brown_1] positioned -25 63.75 2 run loot replace entity @e[type=minecraft:armor_stand, tag=field_menu, tag=rotate_slow] armor.head loot mineopoly:get_head
...
execute if score @s currentField matches 1 positioned -25 63.75 2 run tp @e[type=minecraft:creeper, tag=menu_rent] ~ ~ ~-1

# brown_2
execute if score @s currentField matches 3 positioned -25 63.75 36 run summon armor_stand ~ ~ ~ {Invisible:1b,Invulnerable:1b,NoGravity:1b,PersistenceRequired:1b,NoBasePlate:1b,Rotation:[270f],CustomName:'{"translate":"mineopoly.streetname.brown_2", "bold": true, "color": "#825432"}',CustomNameVisible:1b,Tags:["field_menu","rotate_slow"],DisabledSlots:4144959}
